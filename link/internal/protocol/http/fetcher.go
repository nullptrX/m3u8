...
import (
	...
	base2 "github.com/nullptrx/v2/link/pkg/base"
...
func (f *Fetcher) Resolve(req *base2.Request) (*base2.Resource, error) {
	...
	httpReq.Header.Set(base2.HttpHeaderRange, fmt.Sprintf(base2.HttpHeaderRangeFormat, 0, 0))
	...
	res := &base2.Resource{
		Req:   req,
		Range: false,
		Files: []*base2.FileInfo{},
	}
	if base2.HttpCodePartialContent == httpResp.StatusCode {
		// 返回206响应码表示支持断点下载
		res.Range = true
		// 解析资源大小: bytes 0-1000/1001 => 1001
		contentTotal := path.Base(httpResp.Header.Get(base2.HttpHeaderContentRange))
		if contentTotal != "" {
	...
	} else if base2.HttpCodeOK == httpResp.StatusCode {
		// 返回200响应码，不支持断点下载，通过Content-Length头获取文件大小，获取不到的话可能是chunked编码
		contentLength := httpResp.Header.Get(base2.HttpHeaderContentLength)
		if contentLength != "" {
	...
	contentDisposition := httpResp.Header.Get(base2.HttpHeaderContentDisposition)
...
func (f *Fetcher) Create(res *base2.Resource, opts *base2.Options) error {
	f.res = res
	f.opts = opts
	f.status = base2.DownloadStatusReady
	return nil
}
...
func (f *Fetcher) Start() (err error) {
	...
	f.status = base2.DownloadStatusStart
	if f.res.Range {
		// 每个连接平均需要下载的分块大小
...
func (f *Fetcher) Pause() (err error) {
	if base2.DownloadStatusStart != f.status {
		return
	}
	f.status = base2.DownloadStatusPause
	f.cancel()
	<-f.pauseCh
	return
}
...
func (f *Fetcher) Continue() (err error) {
	if base2.DownloadStatusStart == f.status || base2.DownloadStatusDone == f.status {
		return
	}
	f.status = base2.DownloadStatusStart
	var name = f.filename()
...
func (f *Fetcher) fetch() {
	...
	go func() {
		...
		if f.status == base2.DownloadStatusPause {
			f.pauseCh <- nil
		} else {
			if err != nil {
				f.status = base2.DownloadStatusError
			} else {
				f.status = base2.DownloadStatusDone
			}
			f.DoneCh <- err
		}
	}()
...
func (f *Fetcher) fetchChunk(index int) (err error) {
	...
	for i := 0; i < 5; i++ {
		// 如果下载完成直接返回
		if chunk.Status == base2.DownloadStatusDone {
			return
		}
		// 如果已暂停直接跳出
		if f.status == base2.DownloadStatusPause {
		...
		if f.res.Range {
			httpReq.Header.Set(base2.HttpHeaderRange,
				fmt.Sprintf(base2.HttpHeaderRangeFormat, chunk.Begin+chunk.Downloaded, chunk.End))
		} else {
			chunk.Downloaded = 0
		}
		err = func() error {
			...
			if resp.StatusCode != base2.HttpCodeOK && resp.StatusCode != base2.HttpCodePartialContent {
	...
	if f.status == base2.DownloadStatusPause {
		chunk.Status = base2.DownloadStatusPause
	} else if chunk.Downloaded >= chunk.End-chunk.Begin+1 {
		chunk.Status = base2.DownloadStatusDone
	} else {
		if err != nil {
			chunk.Status = base2.DownloadStatusError
		} else {
			chunk.Status = base2.DownloadStatusDone
		}
	}
...
func buildRequest(ctx context.Context, req *base2.Request) (httpReq *http.Request, err error) {
